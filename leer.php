...
<head>
    ...
    $miconsulta = "SELECT * FROM libros WHERE Titulo='$id' ";
...
if($resultado=mysqli_query($miconexion,$miconsulta)){
    while($registro = mysqli_fetch_array($resultado)){
        ...
        if($registro['Imagen']!=""){
            ...
            echo "<img alt='".$registro['Titulo'] ."' src='fx-contenido/img/libros-leer/". $registro['Imagen'] ."' ></a>";
            echo "</center></div>";
        }
        echo "<p class='content3-post'>" . $registro["Descripcion"] . "</p>";
        echo '<embed src="fx-contenido/pdf/'.$registro["PDF"].'" type="application/pdf" class="pdf" frameborder="0" />';
...
<center>
    <?php include "fx-includes/regresar-btn3.php"; ?>
</center>
