...
if (isset($_POST["libros"])) {
    ...
    if($_FILES['imagen']['error']){
        ...
        if ((isset($_FILES['imagen']['name'])&&($_FILES['imagen']['error']==UPLOAD_ERR_OK))){
            ...
            $destino_de_ruta ="../fx-contenido/img/libros-leer/";
    ...
    if($_FILES['PDF']['error']){
        ...
        switch($_FILES['PDF']['error']){
            ...
            break;
            case 4: //No se subio el fichero
                echo "No se envió archivo de PDF";
            break;
        }

    }else{
        echo "<br>Entrada correcta<br>";
        if ((isset($_FILES['PDF']['name'])&&($_FILES['PDF']['error']==UPLOAD_ERR_OK))){

            $destino_de_ruta ="../fx-contenido/pdf/";

            move_uploaded_file($_FILES['PDF']['tmp_name'],$destino_de_ruta . $_FILES['PDF']['name']);

            echo "El archivo " . $_FILES['PDF']['name'] . " se ha copiado en el directorio de PDF";
        }else{
            echo "El archivo no se ha podido copiar a PDF";
        }
    }
    ...
    $myconsulta ="INSERT INTO LIBROS (Titulo, Fecha, Descripcion, Imagen, Seo_d, PDF ) 
    VALUES ('". $Titulo ."','". $fecha ."','". $descripcion ."', '". $imagen ."','".$seo_d."','".$pdf."')";
...
<table>
                ...
                <tr>
                    <td>Descripcion: <label for="area_descripcion"></label></td>
                ...
                <tr>
                    <td colspan="2" align="center">Seleccione el pdf con tamaño inferior a 2 MB</td>
                </tr>
                <tr>
                    <td colspan="2" align="left"><input type="file" name="PDF" id="PDF" /></td>
                </tr>
                <tr>
                ...
                <tr>
                    <td colspan="2" align="center">
                        <input type="submit" name="libros" id="btn_enviar" value="Enviar" />
                    </td>
                </tr>
                <tr>
                    <td colspan="2" align="center"><a href="../libros">Página de visualización de libros</a></td>
                </tr>
            </table>
</form>
